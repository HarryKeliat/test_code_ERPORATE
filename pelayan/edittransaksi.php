<?php
	...
	include "../koneksi.php";
	$id = $_GET['id'];
	$data_edit = mysqli_query($conn, "SELECT * FROM db_transaksi WHERE id = '".$id."'");
...
<body>
		...
		<div class="col-lg-6">
		    <div class="col-sm-9 col-md-7 col-lg-9 mx-auto" style=" margin-top: 100px; background-image: url('Image/back.jpg');">
		    		<h3 style="font-size: 35px; font-weight: bold;color: white;">Edit Data Pemesanan</h3>
		    		<div id="myAlert" class="alert alert-success collapse">
						<a id="linkClose" href="" class="close">&times;</a>
						<strong>Pemesanan berhasil di rubah !!!</strong>
					</div>
					<div id="myAlert2" class="alert alert-danger collapse">
						<a id="linkClose" href="" class="close">&times;</a>
						<strong>Pemesanan gagal di rubah !!!</strong>
					</div>
		    		<form action="" method="post" enctype="multipart/form-data">
						<div class="form-row">
						    <div class="form-group col-md-6">
						      <label style="color: white;">No. Pemesanan</label>
							  <input type="text" class="form-control" required name="pemesanan" value="<?php echo $row['pemesanan'] ?>" readonly>
						    </div>
						    <div class="form-group col-md-3">
						      <label style="color: white;">Nama Pelayan</label>
						      <input type="text" class="form-control" required name="pelayan" autocomplete="off" value="<?php echo $row['pelayan'] ?>" readonly>
						    </div>
						    <div class="form-group col-md-3">
						      <label style="color: white;">Status</label>
						      <input type="text" class="form-control" required name="status" autocomplete="off" value="<?php echo $row['status'] ?>" readonly>
						    </div>
						</div>
						<div class="form-row">
						    <div class="form-group col-md-3">
						      <label style="color: white;">No. Meja</label>
						      <input type="number" class="form-control" required name="nomer" autocomplete="off" value="<?php echo $row['nomer'] ?>" readonly>
						    </div>
						    <div class="form-group col-md-9">
						      <label style="color: white;">Nama Pelanggan</label>
						      <input type="text" class="form-control" required name="nama" autocomplete="off"value="<?php echo $row['nama'] ?>" readonly>
						    </div>
						</div>
						...
						<div class="form-group">
					  	...
					  	<button type="submit" name="masuk" class="btn btn-success" style="width: 100%; margin-bottom: 20px;">Edit Transaksi Pemesenan</button>
					  	<?php 
					  		include "../koneksi.php";
							if (isset($_POST['masuk'])) {
								$edit = mysqli_query ($conn, "UPDATE db_transaksi SET
								isi= '".$_POST['isi']."'
								WHERE id= '".$id."'LIMIT 1"); 
								if($edit)
								{  
